...
description: "How to write a custom profile importer."
---

# Writing a Custom Profile Importer

The Firefox Profiler supports a few [external profile formats](../src/profile-logic/import) that the profiler imports by converting them to either the [Gecko profile format](/developer/gecko-profile-format) or the [processed profile format](/developer/processed-profile-format). The good news is that once an importer targets a specific profile version, the Firefox Profiler will always upgrade that profile to the currently supported format.

## Useful Docs

- [Gecko profile format docs](/developer/gecko-profile-format)
